...
export const quarterService = {
  ...
  async create(quarter: Quarter,fincaId:number): Promise<Quarter> {

    const response = await axios.post<Quarter>('/api/cuarteles', {...quarter,fincaId});
    return response.data;
  },

  async update(quarter: Quarter,fincaId:number): Promise<Quarter> {
    const response = await axios.put<Quarter>(`/api/cuarteles/${quarter.id}`, {...quarter,fincaId});
    return response.data;
  },
